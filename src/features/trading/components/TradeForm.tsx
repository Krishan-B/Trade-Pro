...
const TradeForm = ({
  ...
  // Use a type for the error fields only
  interface TradeFormErrorFields {
    units?: string[];
    rate?: string[];
    stopLoss?: string[];
    takeProfit?: string[];
  ...
  const [formErrors, setFormErrors] = useState<
    TradeFormErrorFields | undefined
  >();
  ...
  const filteredAssets =
    marketData?.filter((a) => a.market_type === assetCategory) || [];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setFormErrors(undefined); // Clear previous errors
    ...
    <form onSubmit={handleSubmit} className="space-y-4 py-2">
      ...
      {orderType === "entry" && (
        <div className="mb-4">
          ...
          {formErrors?.rate && (
            <p className="text-red-500 text-xs mt-1">{formErrors.rate[0]}</p>
          )}
