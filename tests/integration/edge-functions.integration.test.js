...
import { assertAuthSuccess, assertSuccess, } from "../utils/error-utils";
import { assertStorageSuccess } from "../utils/storage-utils";
const SUPABASE_URL = process.env.VITE_SUPABASE_URL || "http://127.0.0.1:54321";
const SUPABASE_ANON_KEY = process.env.VITE_SUPABASE_ANON_KEY || "your-local-anon-key";
function assertEdgeFunctionSuccess(response) {
...
function assertEdgeFunctionError(response) {
    expect(response).toBeDefined();
    expect(response).not.toBeNull();
    const resp = response;
...
describe("Edge Functions", () => {
    ...
    test("should calculate position metrics", async () => {
        const { data, error } = await supabase.functions.invoke("calculate-position-metrics", {
            body: {
                symbol: "BTC/USD",
                entryPrice: 50000,
                currentPrice: 55000,
    ...
    test("should validate KYC documents", async () => {
        ...
        // Use storage assertion for storage upload
        assertStorageSuccess({ data: uploadData, error: uploadError });
        // Then trigger the validation
        const { data, error } = await supabase.functions.invoke("validate-kyc-document", {
            body: {
                userId,
    ...
    test("should handle market data processing", async () => {
        const { data, error } = await supabase.functions.invoke("process-market-data", {
            body: {
                symbol: "ETH/USD",
                price: 2000,
    ...
    test("should process batch market data", async () => {
        const { data, error } = await supabase.functions.invoke("process-market-data-batch", {
            body: {
                updates: [
                    {
                        symbol: "BTC/USD",
