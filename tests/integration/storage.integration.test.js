...
async function createTestFile(content, name, type = "text/plain") {
    return new File([content], name, { type });
}
async function cleanupStorage(supabase, bucket, prefix) {
...
describe("Storage Operations", () => {
    ...
    beforeAll(async () => {
        ...
        // Create test bucket
        const { error: bucketError } = await supabase.storage.createBucket(testBucket, {
            public: false,
        });
        if (bucketError)
            throw bucketError;
    ...
    test("should handle concurrent uploads", async () => {
        ...
        const results = await Promise.all(files.map(({ path, content }) => supabase.storage.from(testBucket).upload(path, content)));
