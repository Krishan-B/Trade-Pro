...
export class DisposableRegistry {
    ...
    track(disposable, meta) {
        if (meta && meta.context && meta.lifecyclePhase) {
            ...
            if (meta.lifecyclePhase === "early-init") {
                console.warn(`[DisposableRegistry] ⚠️ Registered during risky phase: '${meta.lifecyclePhase}' in context: ${meta.context}`);
            }
        }
        if (this.traceEnabled) {
            ...
            if (count + 1 > this.warnThreshold) {
                console.warn(`[DisposableRegistry] Hotspot: ${count + 1} disposables from:\n${stack}`);
            }
        }
    ...
    warnIfAbove(threshold) {
        ...
        [...this.heatmap.entries()]
            ...
            .forEach(([stack, count]) => {
            warned = true;
            console.warn(`[DisposableRegistry] WARNING: ${count} disposables from:\n${stack}\n`);
        });
        if (!warned) {
            console.log(`[DisposableRegistry] All disposable callsites are below threshold (${threshold}).`);
        }
    }
    traceLeaks(threshold) {
        if (!this.traceEnabled)
            return;
    ...
    warnIfTooMany(threshold = 3) {
        ...
        for (const [key, count] of counts) {
            if (count > threshold) {
                console.warn(`[DisposableRegistry] ⚠️ High listener count (${count}) for ${key}`);
            }
        }
    }
}
